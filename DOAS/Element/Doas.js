...
function processDoasControl(data, uiCommands = {}, stateStorage = {}) {
    try {
        ...
        const controlResult = {
            ...
            gasValvePosition: 0,         // AO2: 0-10V gas valve control (0-100%)
        ...
        const supplyTemp = parseSafeNumber(data.AI4,
        ...
        const spaceTemp = parseSafeNumber(data.AI2,
        ...
        let targetSetpoint = DOAS_CONFIG.DEFAULT_SETPOINT;
            ...
            if (inDehumidificationMode) {
                ...
                if (allowCooling && !coolingLockout) {
                    ...
                    if (allowHeating && !heatingLockout) {
                        ...
                        if (tempError > 0) {
                            const calculatedPosition = Math.min(DOAS_CONFIG.GAS_VALVE_MAX,
                                Math.max(DOAS_CONFIG.GAS_VALVE_MIN, tempError * 10));
                            controlResult.gasValvePosition = Math.max(calculatedPosition, DOAS_CONFIG.GAS_VALVE_LOW_FIRE_MIN);
                        } else {
                            controlResult.gasValvePosition = DOAS_CONFIG.GAS_VALVE_LOW_FIRE_MIN;
                        }
                    }
                }
            } else {
                ...
                if (shouldHeat) {
                    ...
                    if (tempError > 0) {
                        controlResult.heatEnable = true;
                        controlResult.gasValvePosition = Math.min(DOAS_CONFIG.GAS_VALVE_MAX,
                            Math.max(DOAS_CONFIG.GAS_VALVE_MIN, tempError * 10));
                    } else {
                        controlResult.heatEnable = false;
                        controlResult.gasValvePosition = 0;
        ...
        console.log(`[Element DOAS] Control processed: Mode=${controlResult.controlMode}, Fan=${controlResult.oaDamperFanEnable}, Heat=${controlResult.heatEnable}, Gas=${controlResult.gasValvePosition}%, Cool1=${controlResult.chillerStage1Enable}, Cool2=${controlResult.chillerStage2Enable}, Supply=${supplyTemp.toFixed(1)}°F, OAT=${outdoorTemp.toFixed(1)}°F, Occupied=${isOccupied}`);
